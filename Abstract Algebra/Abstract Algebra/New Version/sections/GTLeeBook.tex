...
\section{Ideas, Factor Rings, Homomorphisms}
\input{Images/correspondenceTheorem.tex }


\begin{problem}
	List all of the elements in each of the following principal ideals of $ \Z_4 \oplus \Z_6 $.
	\begin{enumerate}[(a)]
		\item $ ((0,5)) $.
		\item $ ((2,3)) $.
	\end{enumerate}
...
\begin{solution}
	\begin{enumerate}[(a)]
		\item $ ((0,5)) = \set{(0,0),(0,5),(0,4),(0,3),(0,2),(0,1)} = \set{0} \oplus \Z_6 $.
		
		\item $ ((2,3)) = \set{(0,0),(2,0),(0,2),(2,3)}. $
	\end{enumerate}
\end{solution}

\begin{problem}
	Let $ I $ be the set of all polynomials in $ \Z[x] $ whose constant term is a multiple of $ 5 $. Show that $ I $ is an ideal of $ \Z[x] $.
\end{problem}
\begin{solution}
	Let $ f(x) \in I $. Then we have $ f(0) =0 \mod5 $. Let $ g(x) \in \Z[x] $. Then $ g(0) f(0) = \mod5 $. So $ I $ is an ideal.
\end{solution}
...
\end{problem}
\begin{solution}
	Let $ r\in I \cap J $. Then for all $ s\in R $ we have $  rs \in I $ and $ rs \in J $. So $ rs \in I \cap J $. So $ I\cap J $ is an ideal. If $ K $ is another ideal, then $  \cap J \cap K$ is also an ideal. To see this first notice that $  I\cap J$ is an ideal, thus $ (I\cap J) \cap K $ is an ideal. By induction, the intersection of an arbitrary collection of ideals is an ideal.
\end{solution}

\begin{problem}
	Let $ m $ and $ n $ be positive integers. Show that $ m\Z + n\Z = (m,n) \Z $.
\end{problem}
\begin{solution}
	Observe that for any $ m,n\in \Z $, exists $ a,b\in \Z $ such that $ am + bn = (m,n) $. So
	\[ (m,n)\Z = (am + bn)\Z = am\Z + bn\Z = m\Z + n\Z. \]
\end{solution}

\begin{problem}
	Given a ring $ R $ and an element $ a $, let $ S = \set{ra:r\in R} $. Show by example that
	\begin{enumerate}[(a)]
		\item $ S $ need not contain $ a $, even if $ \R $ is commutative.
		\item $ S $ need no be an ideal, even if $ R $ has an identity.
	\end{enumerate}
\end{problem}
\begin{solution}
	\begin{enumerate}[(a)]
		\item This can happen if the underlying ring does not have an identity. For instance, consider the ring $ R = 2\Z $. Then the principal ideal $ (2) = 4\Z $ does not contain $ 2 $.
		
		\item This can happen if $ R $ is not commutative, like in the ring of matrices. In this case, one needs to talk about the left and right ideals.
	\end{enumerate}
\end{solution}

\begin{problem}
	Let $ R $ be a commutative ring. If $ I = \set{a\in R: a^n = 0\text{ for some $ n\in \N $}} $, show that $ I $ is an ideal of $ R $.
\end{problem}
\begin{solution}
	Let $ r\in I $ and $ s\in R $. Then we claim that $ sr \in I $. Because $ (sr)^n = s^n r^n = s^n 0 = 0 $. 
\end{solution}


\begin{problem}
	Let $ R $ be a commutative ring with identity and having exactly two ideals. Show that $ R $ is a field.
\end{problem}
\begin{solution}
	Let $ a\in R $, and consider the principal ideal  $ (a) $. $ (a) $ is not empty, and since $ R $ has only two ideals, it follows that $ (a) = R $. Since $ R $ has identity, it implies that $ \exists b\in R $ such that $ ab = 1 $. So $ a $ has an inverse, hence $ R $ is a field.
\end{solution}


\begin{problem}
	Consider the additive group $ G $ of all sequences integers with element-wise addition, and the subgroup $ H $ of all elements of $ G $ that has finitely many non-zero elements (have finite support). Define a multiplication operation by element wise multiplication. Show that $ G $ is a ring and $ H $ is an ideal.
\end{problem}

\begin{solution}
	$ G $ being a ring follows from $ \Z $ having a ring structure. Furthermore, $ H $ is an ideal because if $ a\in H $ and $ b\in G $ then $ ab $ still has finite support. So $ ab\in H $.
\end{solution}
...
\end{problem}
\begin{solution}
	Assume $ H $ is a principal ideal. Then $ H = (g) $ for some $ g\in G $ that has finite support. However, the support of all elements of $ H $ is the same as $ g $, which is not true. Indeed, let $ \tilde{g} $ such that its support is larger than $ g $ (has more non-zero terms). By definition $ \tilde{g} \in H $ but there is no $ r\in G $ that $ \tilde{g} = r g $.
\end{solution}
