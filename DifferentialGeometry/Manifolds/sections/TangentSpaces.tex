...
\begin{proposition}[Special Linear Group is a Manifold]
...
\begin{observation}[Surjectivity implies maximal rank, but the converse is not true!]
...
So for a smooth map between manifolds, say $ F: N\to M $, assuming that it has maximal rank at $ p \in N $, we will have the following three cases that are not mutually exclusive:
\begin{enumerate}[(i)]
...
\begin{theorem}[Constant Rank Theorem for Euclidean Spaces]
...
\begin{example}[A useful characterization of constant rank for a map]
...
This idea of the constant rank theorem can easily be extended to the manifolds with almost no cost. The following proposition captures this idea.

\begin{proposition}[Constant Rank Theorem for Manifolds]
...
\begin{proposition}[Constant Rank Level set Theorem]
...
\begin{proposition}[The orthogonal group $ O(n) $ is a manifold.]
...
\begin{observation}[Submersion and Immersion Implies Maximal Rank]
...
\begin{observation}[Rank Maximality is an open Condition]
...
\begin{proposition}[Normal Forms for Immersion and Submersion]
