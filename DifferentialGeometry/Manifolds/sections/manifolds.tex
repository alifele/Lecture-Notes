...
\begin{proof}
	...
	where $ (x_1,y_1,0), (x_2,y_2,0) \in H^2 $. Thus the proposition states that the two spaces $ H^2/\sim_1 $ is homeomorphic to $ S^2/\sim $. To prove this we used the idea as in \autoref{summary:ShowingHomeoInQuotientSpaces}. Thus we need to find a continuous map $ f: S^2 \to  H^2$ that induces a continuous map $ \bar{f}: S^2/\sim \to H^2/\sim_1 $. The map $ f $ is given by
	\[ f(x,y,z) = \begin{cases}
		\sign(z)(x,y,z) \qquad z\neq 0,\\
		(x^2 - y^2, 2xy,0) \qquad z=0.
	\end{cases} \]
	This is indeed a continuous map on its domain. The continuoity is clear for the case $ z\neq 0 $. For 
\end{proof}
...
\begin{summary}[Showing two quotient spaces are homeomorphic]
	\label{summary:ShowingHomeoInQuotientSpaces}
	In general, in order to show that two spaces are homeomorphic, we need to find an homeomorphism between the spaces (a continuous bijection with continuous inverse). In particular, if two spaces are quotient spaces, it is often easier to work with the original spaces (that are not necessarily homeomorphic). For instance, Consider showing that the real projective space $ \R P^n $ is homeomorphic to $ S^n/\sim $ where the equivalence relation $ \sim $ identifies the antipodal points in the unit sphere. The followings are the steps that we take
	\begin{itemize}
		...
		\begin{center}
			\includegraphics[width=0.3\linewidth]{Images/conuttativeDiagranQuestion1.png}
		\end{center}
		\item We need to find $ g: S^n \to \R^n-\set{0}  $ that induces a map $ \bar{g}: S^n/\sim \to \R^n-\set{0}/\sim  $. Again, using \autoref{prop:ContinousInducedmap}, $ \bar{g} $ is continuous if and only if $ \pi_1\circ g $ is continuous. 
