...
\end{observation}
	\input{Images/diffeoMorphVsHomeomorph.tex}
...
\end{observation}
	\input{Images/InverseFunctiontheoremProofIdea.tex}
...
\end{proposition}
\begin{proof}
	First, we need to write the proposition in terms of actual mathematical objects (sets, mappings, etc). Let
	\[ H^2 = \set{(x,y,z) \in \R^3\ :\ x^2+y^2+z^2 = 1,\ z\geq 0}. \]
	Define the equivalence relation 
	\[ (x_1,y_1,0)\  \sim_{1}\  (x_2,y_2,0) \quad \Longleftrightarrow \quad (x_1,y_1,0)=\pm (x_2,y_2,0) \]
	where $ (x_1,y_1,0), (x_2,y_2,0) \in H^2 $. Thus the proposition states that the two spaces $ H^2/\sim_1 $ is homeomorphic to $ S^2/\sim $. To prove this we used the idea as in \autoref{summary:ShowingHomeoInQuotientSpaces}. Consider the inclusion map $ i: H^2 \to S^2 $. This map is continuous and $ \pi_2\circ i $ assumes a constant value for all of the points on its domain that are in the same equivalence class. Thus it induces a continuous map $ f: H^2/\sim_1 \to S^2/\sim $. The following commutative diagram summarizes this.
%	\input{Images/diagraForQuestion1.tex}
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJIXjIiXSxbMiwwLCJTXjIiXSxbMCwyLCJIXjIvXFxzaW0iXSxbMiwyLCJTXjIvXFxzaW0iXSxbMCwxLCJpIl0sWzAsMiwiXFxwaV8xIiwyXSxbMSwzLCJcXHBpXzIiXSxbMiwzLCJmIiwyXSxbMCwzLCJcXHBpXzJcXGNpcmMgaSIsMV1d
	\[\begin{tikzcd}
		{H^2} && {S^2} \\
		\\
		{H^2/\sim} && {S^2/\sim}
	...
	\noindent To find the inverse map, consider the map $ g: S^2 \to H^2 $ given by
	\[ k(x,y,z) = (x,y,\abs{z}). \]
	This map is continuous (as all of its component functions are continuous) and $ \pi_1\circ k $ assumes a constant value for all of points in its domain that are in the same equivalence class. This induces a continuous map $ g: S^2/\sim \to H^2/\sim_1 $. Consider the following commutative diagram that summarizes these functions.
%	\input{Images/diagraForQuestion2.tex}
	% https://q.uiver.app/#q=WzAsNCxbMiwwLCJIXjIiXSxbMCwwLCJTXjIiXSxbMiwyLCJIXjIvXFxzaW0iXSxbMCwyLCJTXjIvXFxzaW0iXSxbMCwyLCJcXHBpXzEiXSxbMSwzLCJcXHBpXzIiLDJdLFsxLDAsImsiXSxbMSwyLCJcXHBpXzFcXGNpcmMgayIsMV0sWzMsMiwiZyJdXQ==
	\[\begin{tikzcd}
		{S^2} && {H^2} \\
		\\
		{S^2/\sim} && {H^2/\sim}
		...
		\arrow["{\pi_1\circ k}"{description}, from=1-1, to=3-3]
		\arrow["{\pi_1}", from=1-3, to=3-3]
		\arrow["g", from=3-1, to=3-3]
	\end{tikzcd}\]
	
	Now what remains to show is that $ f $ and $ g $ are inverse of each other. To show this consider
	\[ f\circ g: S^2/\sim \to S^2/\sim. \]
	Let $ [(x,y,z)] \in S^2/\sim $. Then we have
	\[ f(g([x,y,z])) = f((\pi_1\circ k)(x,y,z)) = f([x,y,\abs{z}])\]
	
	
	
%	 where $ \pi_2\circ f $ assumes a constant value for all of the points in a single equivalence class on its domain. Consider the identity map $ \mathds{1}_{S^2}: S^2 \to H^2 $. This map is indeed continuous and the map $ \pi_2\circ \pi_1 $
\end{proof}
...
\begin{summary}[Two ways to show that two quotient spaces are homeomorphic]
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			Also look at the proof of \autoref{prop:UnitSphereHomoToUpperHemisphere} for a similar argument.
			
		\end{itemize}
		
		\item \textbf{Using compactness arguments}. Some times it is hard to find the homeomorphism directly. In this case we follow our approach as in the problem {\color{orange} (problem and problem here)}, as well as the following important proposition
		\begin{proposition}
...
\input{sections/Manifold_SolvedProbelms.tex}