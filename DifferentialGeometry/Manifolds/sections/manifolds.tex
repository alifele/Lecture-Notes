...
\begin{proof}
	...
	where $ (x_1,y_1,0), (x_2,y_2,0) \in H^2 $. Thus the proposition states that the two spaces $ H^2/\sim_1 $ is homeomorphic to $ S^2/\sim $.
\end{proof}
...
\begin{summary}[Showing two quotient spaces are homeomorphic]
	...
	\begin{itemize}
		\item Find $ f: \R^n-\set{0} \to S^n $ that assumes a constant value for all of the points in the same equivalence relation. This induces a map $ \bar{f}: \R P^n \to S^n/\sim $. From \autoref{prop:ContinousInducedmap} it follows that $ \bar{f} $ is continuous iff $ \pi^2 \circ f $ is continuous (consider the following commutative diagram).
		\begin{center}
			\includegraphics[width=0.3\linewidth]{Images/conuttativeDiagranQuestion1.png}
		\end{center}
