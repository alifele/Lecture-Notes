...
\begin{problem}[Charts on a sphere (from W. Tu)]
	...
	\end{align*}
	Note that although some of the functions above might look similar (like $ \phi_3 $ and $ \phi_4 $) but they are in fact different functions as they have different domains. Describe the domain $ \phi_4(U_1 \cap U_4) $ of $ \phi_1\circ \inv{\phi_4} $ and show that $ \phi_1\circ \inv{\phi_4} $ is $ C^\infty $ on $ \phi_4(U_1 \cap U_4) $. Do the dame for $ \phi_6 \circ \inv{\phi_1}$
...
\begin{solution}
	...
	Thus we will have
	\[ \phi_4(U_1\cap U_4) = \set{(x,z)\in \R^2 \ :\ x^2 + z^2 \leq 1,\ x>0 \text{ and } z<0}. \]
	Thus we can write
	...
	\[ \phi_1(U_1 \cap U_4) = \set{(z,y)\in\R^2\ :\ z^2 + y^2 < 0,\ z<0 \text{ and } y < 0}. \]
\end{solution}