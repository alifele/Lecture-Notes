...
\begin{problem}
	Prove that the monotone convergence theorem is equivalent to the nested interval theorem. The following states the nested interval theorem. Given a sequence of nested intervals $[a_n, b_n]$ such that $a_n < b_n$, and has the following arrangement 
...
\begin{proof}
	~\vspace{2pt}
	\begin{enumerate}[(i)]
		\item \textbf{The monotone convergence theorem implies the nested intervals theorem.} Assume that we have the nested intervals with its length shrinking to zero, then we want to show that there is only one element at the intersection.
